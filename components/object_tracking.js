	...
	computed: {
		object_tracks: function () {
			...
			for (
				...
				if ('objectAnnotations' in this.json_data.annotationResults[index])
					return this.json_data.annotationResults[index].objectAnnotations
			}
			return []
		},
...
class Object_Track {
	constructor(json_data, video_height, video_width) {
		...
		json_data.frames.forEach((frame) => {
			const new_frame = {
				box: {
					x: (frame.normalizedBoundingBox.left || 0) * video_width,
					y: (frame.normalizedBoundingBox.top || 0) * video_height,
					width:
						((frame.normalizedBoundingBox.right || 0) -
							(frame.normalizedBoundingBox.left || 0)) *
						video_width,
					height:
						((frame.normalizedBoundingBox.bottom || 0) -
							(frame.normalizedBoundingBox.top || 0)) *
						video_height,
				},
