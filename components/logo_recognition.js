...
Vue.component('logo-recognition-viz', {
	...
	computed: {
		logo_tracks: function () {
			...
			if (!this.json_data.annotationResults) return []

			for (
				let index = 0;
				index < this.json_data.annotationResults.length;
				index++
			) {
				if (
					'logoRecognitionAnnotations' in
					this.json_data.annotationResults[index]
				)
					return this.json_data.annotationResults[index]
		...
		indexed_logo_tracks: function () {
			...
			this.logo_tracks.forEach((element) => {
				const detected_logo = new Logo_Detected(
					element,
					this.video_info.height,
					this.video_info.width,
		...
		object_track_segments: function () {
			...
			this.indexed_logo_tracks.forEach((object_tracks) => {
				...
				object_tracks.segments.forEach((logo_segemnt) => {
					...
					for (
						let index = 0;
						index < segments[object_tracks.name].length;
						index++
					) {
						const segment = segments[object_tracks.name].segments[index]
						if (logo_segemnt.start_time < segment[1]) {
							segments[object_tracks.name].segments[index][1] = Math.max(
								segments[object_tracks.name].segments[index][1],
								logo_segemnt.end_time
							)
							added = true
							break
						}
					...
					if (!added)
						segments[object_tracks.name].segments.push([
							logo_segemnt.start_time,
							logo_segemnt.end_time,
						])
				})
			})
	...
	methods: {
		segment_style: function (segment) {
			return {
				...
				width:
					(
						((segment[1] - segment[0]) / this.video_info.length) *
						100
					).toString() + '%',
			}
		},
...
class Logo_Frame {
	constructor(json_data, video_height, video_width) {
		this.time_offset = nullable_time_offset_to_seconds(json_data.timeOffset)

		this.box = {
			x: (json_data.normalizedBoundingBox.left || 0) * video_width,
			y: (json_data.normalizedBoundingBox.top || 0) * video_height,
			width:
				((json_data.normalizedBoundingBox.right || 0) -
					(json_data.normalizedBoundingBox.left || 0)) *
				video_width,
			height:
				((json_data.normalizedBoundingBox.bottom || 0) -
					(json_data.normalizedBoundingBox.top || 0)) *
				video_height,
		}
	}
}
...
class Logo_Track {
	constructor(json_data, video_height, video_width) {
		this.start_time = nullable_time_offset_to_seconds(
			json_data.segment.startTimeOffset
		)
		this.end_time = nullable_time_offset_to_seconds(
			json_data.segment.endTimeOffset
		)
		this.confidence = json_data.confidence

		this.frames = []

		json_data.timestampedObjects.forEach((frame) => {
		...
		for (let index = 0; index < this.frames.length; index++) {
			if (this.frames[index].time_offset > seconds) {
				if (index > 0) {
					...
					const time_delt_ratio =
						(seconds - start_box.time_offset) /
						(end_box.time_offset - start_box.time_offset)

					const interpolated_box = {
						x:
							start_box.box.x +
							(end_box.box.x - start_box.box.x) * time_delt_ratio,
						y:
							start_box.box.y +
							(end_box.box.y - start_box.box.y) * time_delt_ratio,
						width:
							start_box.box.width +
							(end_box.box.width - start_box.box.width) * time_delt_ratio,
						height:
							start_box.box.height +
							(end_box.box.height - start_box.box.height) * time_delt_ratio,
					}
...
class Logo_Detected {
	constructor(json_data, video_height, video_width, confidence_threshold) {
		...
		this.id = json_data.entity.entityId
	...
	has_frames_for_time(seconds) {
		for (let index = 0; index < this.segments.length; index++) {
			if (this.segments[index].has_frames_for_time(seconds)) return true
		}
		return false
	}
