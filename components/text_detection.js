	...
	computed: {
		text_tracks: function () {
			...
			for (
				...
				if ('textAnnotations' in this.json_data.annotationResults[index])
					return this.json_data.annotationResults[index].textAnnotations
			}
			return []
		},
		...
		indexed_text_tracks: function () {
			...
			this.text_tracks.forEach((element) => {
				...
				const text_detection = new Text_Detection(
					element,
					this.video_info.height,
					this.video_info.width,
					this.confidence_threshold
				)
				if (text_detection.segments.length) indexed_tracks.push(text_detection)
			})
...
function draw_bounding_polys(object_tracks, ctx) {
	...
	object_tracks.forEach((tracked_object) => {
		if (tracked_object.has_frames_for_time(current_time)) {
			draw_bounding_poly(
				tracked_object.current_bounding_box(current_time),
				tracked_object.text,
				ctx
			)
		}
...
class Text_Frame {
	constructor(json_data, video_height, video_width) {
		...
		json_data.rotatedBoundingBox.vertices.forEach((vertex) => {
...
class Text_Segment {
	...
	most_recent_real_poly(seconds) {
		for (let index = 0; index < this.frames.length; index++) {
			if (this.frames[index].time_offset > seconds) {
				if (index > 0) return this.frames[index - 1].poly
				else return null
			}
		}
		return null
			...
			if (this.frames[index].time_offset > seconds) {
				if (index > 0) {
					...
					const time_delt_ratio =
						(seconds - start_poly.time_offset) /
						(end_poly.time_offset - start_poly.time_offset)
					...
					for (let i = 0; i < 4; i++) {
						interpolated_poly.push({
							x:
								start_poly.poly[i].x +
								(end_poly.poly[i].x - start_poly.poly[i].x) * time_delt_ratio,
							y:
								start_poly.poly[i].y +
								(end_poly.poly[i].y - start_poly.poly[i].y) * time_delt_ratio,
						})
					}
	...
	current_bounding_box(seconds, interpolate = true) {
		if (interpolate) return this.most_recent_interpolated_poly(seconds)
		else return this.most_recent_real_poly(seconds)
	}
}
