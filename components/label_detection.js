...
Vue.component('label-detection-viz', {
	...
	computed: {
		detected_labels: function () {
			...
			for (
				...
				if ('shotLabelAnnotations' in this.json_data.annotationResults[index])
					return this.json_data.annotationResults[index].shotLabelAnnotations
			}
			return []
		},
		...
		indexed_detected_labels: function () {
			...
			if (!this.detected_labels) return []

			this.detected_labels.forEach((element) => {
				const detected_label = new Detected_Label(
					element,
	...
	methods: {
		segment_style: function (segment) {
			return {
				left:
					((segment.start_time / this.video_info.length) * 100).toString() +
					'%',
				width:
					(
						((segment.end_time - segment.start_time) / this.video_info.length) *
						100
					).toString() + '%',
			}
		},
...
class Detected_Label_Segment {
	constructor(segment) {
		...
		this.start_time = nullable_time_offset_to_seconds(
			segment.segment.startTimeOffset
		)
		this.end_time = nullable_time_offset_to_seconds(
			segment.segment.endTimeOffset
		)
	}
...
class Detected_Label {
	constructor(json_data, confidence_threshold) {
		this.name = ''
		if (json_data.categoryEntities)
			this.name += json_data.categoryEntities[0].description + ' '
		...
		json_data.segments.forEach((segment) => {
			if (segment.confidence > confidence_threshold) {
				...
				this.highest_segment_confidence = Math.max(
					this.highest_segment_confidence,
					segment.confidence
				)
			}
		})
	...
	has_segment_for_time(seconds) {
		for (let index = 0; index < this.segments.length; index++) {
			if (this.segments[index].within_time(seconds)) return true
		}
		return false
	}
}
